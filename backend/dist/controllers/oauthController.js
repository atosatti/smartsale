 ...
 */
export const startMercadoLivreAuth = (req, res) => {
    try {
 ...
 */
export const handleMercadoLivreCallback = async (req, res) => {
    try {
        const { code, state } = req.query;
        if (!code) {
            return res.status(400).json({ error: 'Código de autorização não fornecido' });
        } // Trocar código por token (passar state para PKCE)
        const tokenData = await oauthService.exchangeCodeForToken(code, state); // Obter informações do usuário
        ...
        redirectUrl.searchParams.append('ml_expires_in', String(tokenData.expires_in || 21600));
...
export const saveMercadoLivreToken = async (req, res) => {
    try {
       ...
       WHERE id = ?`, [ml_token, ml_user_id, refresh_token || null, expiresAt, userId]);
        res.status(200).json({
            success: true,
 ...
 */
export const disconnectMercadoLivre = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Usuário não autenticado' });
 ...
 */
export const getMercadoLivreStatus = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Usuário não autenticado' });
        }
        const [rows] = await db.query('SELECT mercado_livre_token, mercado_livre_user_id FROM users WHERE id = ?', [userId]);
...
export const createTestUser = async (req, res) => {
    try {
        ...
        if (oauthTokenService.isTokenExpired(token)) {
            if (!refreshToken) {
                res.status(401).json({
            ...
            try {
                ...
                mlToken = newTokenData.access_token;
            }
            catch (refreshError) {
        ...
        try {
            ...
            await pool.query(`UPDATE oauth_tokens SET 
          test_user_id = ?, 
          test_nickname = ?,
          test_password = ?
        WHERE provider = 'mercado-livre' AND is_active = TRUE`, [testUser.id, testUser.nickname, testUser.password]);
            res.status(200).json({
                success: true,
...
export const authenticateTestUser = async (req, res) => {
    try {
        ...
        try {
         ...
         WHERE provider = 'mercado-livre' AND is_active = TRUE`, [nickname, password]);
            res.status(200).json({
                success: true,
...
export const getTestUserInfo = async (req, res) => {
    try {
        let tokens;
        try {
            const [result] = await pool.query('SELECT test_user_id, test_nickname, test_password FROM oauth_tokens WHERE provider = ? AND is_active = TRUE LIMIT 1', ['mercado-livre']);
            tokens = result || [];
        }
        catch (dbError) {
