...
export const register = async (req, res) => {
    try {
        ...
        const verificationToken = generateToken(0, email);
        await pool.query('INSERT INTO users (email, password, first_name, last_name, is_verified, subscription_plan) VALUES (?, ?, ?, ?, ?, ?)', [email, hashedPassword, firstName, lastName, false, 'free']); // Send verification email
        let isVerified = false;
        try {
            await sendVerificationEmail(email, verificationToken);
        }
        catch (emailError) { // In development, auto-verify if email failsawait pool.query('UPDATE users SET is_verified = ? WHERE email = ?', [true, email]);
            isVerified = true;
        }
        res.status(201).json({
            message: isVerified
...
export const confirm2FA = async (req, res) => {
    try {
        ...
        await pool.query('UPDATE users SET two_fa_enabled = true, two_fa_secret = ? WHERE id = ?', [secret, req.user.id]);
...
export const verify2FA = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT id, email, two_fa_secret FROM users WHERE id = ? AND two_fa_enabled = true', [userId]);
        const user = users[0];
        if (!user) {
            return res.status(401).json({ error: 'Invalid or expired token' });
        }
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const updateResult = await pool.query('UPDATE users SET first_name = ?, last_name = ? WHERE id = ?', [first_name.trim(), last_name.trim(), userId]); // Buscar dados atualizados
        const [users] = await pool.query('SELECT id, email, first_name, last_name, subscription_plan, two_fa_enabled FROM users WHERE id = ?', [userId]);
        const user = users[0];
        if (!user) {
...
export const getCurrentUser = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT id, email, first_name, last_name, subscription_plan, two_fa_enabled, role FROM users WHERE id = ?', [userId]);
        const user = users[0];
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
...
export const get2FAStatus = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT two_fa_enabled FROM users WHERE id = ?', [req.user.id]);
        const user = users[0];
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
...
export const disable2FA = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT two_fa_secret, two_fa_enabled FROM users WHERE id = ?', [req.user.id]);
        const user = users[0];
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        ...
        // Desabilitar 2FA
        await pool.query('UPDATE users SET two_fa_enabled = false, two_fa_secret = NULL WHERE id = ?', [req.user.id]);
        res.status(200).json({
            success: true,
...
export const handleGoogleCallback = async (req, res) => {
    try {
        ...
        const [existingUsers] = await pool.query('SELECT * FROM users WHERE google_id = ? OR email = ?', [googleUser.id, googleUser.email]);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            const [newUsers] = await pool.query('SELECT * FROM users WHERE google_id = ?', [googleUser.id]);
            user = newUsers[0];
