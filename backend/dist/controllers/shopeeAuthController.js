 ...
 */
export const startShopeeAuth = (req, res) => {
    try {
...
export const handleShopeeCallback = async (req, res) => {
    try {
        ...
        const tokenResponse = await shopeeOAuthService.exchangeCodeForToken(String(code), String(shop_id));
        // Salvar token no banco (usuário será admin por enquanto)
        // Em um fluxo completo, a loja teria seu próprio usuário
        await shopeeOAuthService.saveToken(tokenResponse.shop_id, 1, // Admin user ID (deverá ser configurado dinamicamente em produção)
        tokenResponse.access_token, tokenResponse.refresh_token, tokenResponse.expire_in);
 ...
 */
export const disconnectShopee = async (req, res) => {
    try {
 ...
 */
export const getShopeeStatus = async (req, res) => {
    try {
 ...
 */
export const getAuthorizedShops = async (req, res) => {
    try {
 ...
 */
export const revokeShopeeAccess = async (req, res) => {
    try {
 ...
 */
export const forceTokenRefresh = async (req, res) => {
    try {
        const { shopId } = req.body;
        if (!shopId) {
            return res.status(400).json({
...
//# sourceMappingURL=shopeeAuthController.js.map