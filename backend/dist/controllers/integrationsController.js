...
export const getIntegrationsStatus = async (req, res) => {
    try {
        const [tokens] = await pool.query(`SELECT id, provider, provider_user_id, is_active, expires_at, last_refreshed_at, created_at, created_by
       FROM oauth_tokens
...
export const saveMercadoLivreToken = async (req, res) => {
    try {
        ...
        await oauthTokenService.saveToken('mercado-livre', ml_token, ml_refresh_token || null, ml_expires_in || undefined, ml_user_id, 'offline_access', req.user.id, req.ip, req.get('user-agent'));
        res.status(200).json({
            success: true,
...
export const refreshMercadoLivreToken = async (req, res) => {
    try {
        ...
        if (token.refresh_token) {
            ...
            // Atualizar token
            await oauthTokenService.refreshToken(token.id, data.access_token, data.refresh_token, data.expires_in, req.user.id, req.ip, req.get('user-agent'));
 ...
 */
export const validateMercadoLivreConnection = async (req, res) => {
    try {
        if (!req.user?.id) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
...
export const disconnectMercadoLivre = async (req, res) => {
    try {
        ...
        await oauthTokenService.disconnectToken('mercado-livre', req.user.id, req.ip, req.get('user-agent'));
        res.status(200).json({
            success: true,
