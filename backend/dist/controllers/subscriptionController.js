...
export const createSubscription = async (req, res) => {
    try {
        ...
        await pool.query('UPDATE users SET subscription_plan = ? WHERE id = ?', [planId, req.user.id]);
        res.status(200).json({
...
export const cancelSubscription = async (req, res) => {
    try {
        ...
        await pool.query('UPDATE users SET subscription_plan = ? WHERE id = ?', ['free', req.user.id]);
        res.status(200).json({
...
export const getUserSubscription = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT id, email, subscription_plan, subscription_status, subscription_start_date, subscription_end_date FROM users WHERE id = ?', [req.user.id]);
        const user = users[0];
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        // Buscar subscription ativa de verdade
        const [subscriptions] = await pool.query(`SELECT id, plan, status, cancel_at_period_end, canceled_at, current_period_end, plan_price
       FROM subscriptions 
       ...
       LIMIT 1`, [req.user.id]);
        ...
        res.status(200).json({
            ...
            plan_details: PLANS[user.subscription_plan],
        });
    }
    catch (error) {
        console.error(error);
...
export const reactivateSubscription = async (req, res) => {
    try {
        ...
        try {
            ...
            const [subscriptions] = await connection.execute(`SELECT id, stripe_subscription_id, plan 
         FROM subscriptions 
