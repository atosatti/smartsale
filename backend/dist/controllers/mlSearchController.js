...
export const getSellerItems = async (req, res) => {
    try {
        ...
        const { 
        // Paginação
        limit, offset, search_type, scroll_id, include_filters, 
        // Status
        status, sub_status, 
        // Ordenação
        orders, 
        // Identificadores
        sku, seller_sku, missing_product_identifiers, 
        // Modo de compra
        buying_mode, 
        // Tipo de listagem
        listing_type_id, 
        // Envio
        shipping_mode, shipping_free_methods, shipping_tags, 
        // Origem
        listing_source, 
        // Saúde
        ...
        const result = await mercadoLivreService.searchSellerItems({
            ...
            shipping_free_methods: shipping_free_methods,
            shipping_tags: shipping_tags,
            // Origem
            listing_source: listing_source,
            // Saúde
            reputation_health_gauge: reputation_health_gauge,
            // Labels
            labels: parsedLabels,
            // Logística
 ...
 */
export const getSellerRestrictions = async (req, res) => {
    try {
        const { sellerId } = req.params;
        if (!sellerId) {
            return res.status(400).json({ error: 'Seller ID é obrigatório' });
        ...
        let userToken;
        if (req.user?.id) {
            const [rows] = await db.query('SELECT access_token FROM oauth_tokens WHERE provider = ? AND is_active = 1 LIMIT 1', ['mercado_livre']);
            if (rows && rows.length > 0) {
                userToken = rows[0].access_token;
            }
        }
        const restrictions = await mercadoLivreService.getSellerRestrictions(parseInt(sellerId), userToken);
        res.status(200).json({
            success: true,
            data: restrictions,
 ...
 * }
 */
export const getItemsMulti = async (req, res) => {
    try {
 ...
 * }
 */
export const getUsersMulti = async (req, res) => {
    try {
        const { ids } = req.body;
        if (!ids || !Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({
 ...
 */
export const searchPublic = async (req, res) => {
    try {
        const { q, seller_id, nickname, category, shipping, condition, sort, limit, offset, } = req.query;
        if (!q) {
            return res.status(400).json({
...
//# sourceMappingURL=mlSearchController.js.map