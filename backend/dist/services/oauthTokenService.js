...
class OAuthTokenService {
     ...
     */
    async saveToken(provider, accessToken, refreshToken, expiresIn, providerUserId, scope, adminUserId, ipAddress, userAgent) {
        try {
            const expiresAt = expiresIn ? new Date(Date.now() + expiresIn * 1000) : null;
            const now = new Date();
            // Usar INSERT ... ON DUPLICATE KEY UPDATE para evitar erro de duplicate
            const [result] = await pool.query(`INSERT INTO oauth_tokens 
         ...
         is_active = TRUE,
         updated_at = NOW()`, [provider, accessToken, refreshToken, 'Bearer', expiresIn, expiresAt, scope, providerUserId, adminUserId, now]);
            // Log na auditoria
            await this.logAuditToken(result.insertId || result.affectedRows, provider, 'connect', `Token do ${provider} atualizado/conectado`, adminUserId, ipAddress, userAgent);
            return {
                id: result.insertId,
                provider,
     ...
     */
    async refreshToken(tokenId, newAccessToken, newRefreshToken, newExpiresIn, adminUserId, ipAddress, userAgent) {
        try {
            const [tokens] = await pool.query(`SELECT * FROM oauth_tokens WHERE id = ?`, [tokenId]);
            ...
            // Atualizar token
            await pool.query(`UPDATE oauth_tokens 
         SET access_token = ?, 
         ...
         WHERE id = ?`, [newAccessToken, newRefreshToken || null, newExpiresIn || null, expiresAt, adminUserId || null, tokenId]);
            // Log na auditoria
            await this.logAuditToken(tokenId, token.provider, 'refresh', 'Token renovado com sucesso', adminUserId || 0, ipAddress, userAgent);
            return {
                ...token,
     ...
     */
    async getActiveTokenWithAutoRefresh(provider) {
        try {
            ...
            try {
                ...
                await this.refreshToken(token.id, data.access_token, data.refresh_token, data.expires_in, 0, // adminUserId = 0 para renovação automática
                '0.0.0.0', // IP para sistema automático
                'auto-renewal'); // Retornar token atualizado
     ...
     * Desconectar token
     */
    async disconnectToken(provider, adminUserId, ipAddress, userAgent) {
        try {
            const [tokens] = await pool.query(`SELECT id FROM oauth_tokens WHERE provider = ? AND is_active = TRUE LIMIT 1`, [provider]);
            const token = tokens[0];
            if (token) {
                await pool.query(`UPDATE oauth_tokens SET is_active = FALSE WHERE id = ?`, [token.id]);
                // Log na auditoria
                await this.logAuditToken(token.id, provider, 'disconnect', 'Token desconectado', adminUserId, ipAddress, userAgent);
            }
        }
        catch (error) {
     ...
     */
    async logAuditToken(tokenId, provider, action, reason, adminUserId, ipAddress, userAgent) {
        try {
            await pool.query(`INSERT INTO oauth_tokens_audit_log 
         (oauth_token_id, provider, action, reason, admin_user_id, ip_address, user_agent)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [tokenId, provider, action, reason, adminUserId, ipAddress || null, userAgent || null]);
        }
        catch (error) {
