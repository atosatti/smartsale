...
export async function notifyAdminCancellation(
  ...
  try {
    ...
    const wouldReturnText =
      wouldReturn === true ? 'Sim' : wouldReturn === false ? 'Não' : 'Não respondido';
 ...
 */
export async function confirmCancellationToUser(userEmail: string, endDate: string) {
  try {
