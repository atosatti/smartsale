 ...
 */
async function getActiveMercadoLivreToken(): Promise<string | null> {
  try {
    let tokens: any[];
    try {
 ...
 */
async function getMercadoLivreAppDetails(req: any, res: Response) {try {
    if (req.user?.role !== 'admin') {return res.status(403).json({ error: 'Acesso negado' });
    }

    const appId = process.env.MERCADO_LIVRE_APP_ID;const token = await getActiveMercadoLivreToken();if (!token) {return res.status(400).json({ error: 'Token do Mercado Livre não configurado' });
    }
      ...
      if (grantsResponse.ok) {
        try {
          ...
          grants = grants.map((grant: any) => {
            ...
            let status = 'Inativo';
            if (daysSinceCreation < 1) status = 'Novo';
            else if (daysSinceCreation <= 90) status = 'Ativo';
    ...
    try {
      ...
      const metricsResponse = await fetch(
        `https://api.mercadolibre.com/applications/v1/${appId}/consumed-applications?date_start=${dateStart}&date_end=${dateEnd}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
 ...
 */
async function getMercadoLivreUserApps(req: any, res: Response) {
  try {
 ...
 */
async function revokeMercadoLivreAuthorization(req: any, res: Response) {
  try {
    if (req.user?.role !== 'admin') {
      return res.status(403).json({ error: 'Acesso negado' });
    }
 ...
 */
async function getMercadoLivreMetrics(req: any, res: Response) {
  try {
    if (req.user?.role !== 'admin') {
      return res.status(403).json({ error: 'Acesso negado' });
    }
