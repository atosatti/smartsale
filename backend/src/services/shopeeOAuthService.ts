...
class ShopeeOAuthService {
  ...
  async saveToken(
    ...
    try {
      ...
      const [existingTokens] = await pool.query(
        'SELECT id FROM oauth_tokens WHERE provider = ? AND provider_user_id = ? LIMIT 1',
        ['shopee', shopId]
      );
      ...
      if ((existingTokens as any[]).length > 0) {
        ...
        [result] = await pool.query(
          ...
          [accessToken, refreshToken, tokenExpiresAt, 'shopee', shopId]
        );
        ...
        [result] = await pool.query(
          `INSERT INTO oauth_tokens 
           ...
           VALUES (?, ?, ?, ?, ?, ?, TRUE, ?, NOW())`,
          ['shopee', accessToken, refreshToken, tokenExpiresAt, shopId, 'products.read,shop.read', userId]
        );
   ...
   */
  async getShopToken(shopId: string): Promise<ShopeeOAuthToken | null> {
    try {
   ...
   */
  async validateAndRefreshToken(shopId: string): Promise<string> {
    try {
      ...
      if (token.token_expires_at.getTime() - now < refreshThreshold) {
        ...
        await pool.query(
          'UPDATE oauth_tokens SET access_token = ?, expires_at = ? WHERE provider = ? AND provider_user_id = ?',
          [newAccessToken, tokenExpiresAt, 'shopee', shopId]
        );
   ...
   */
  async disconnectShop(shopId: string): Promise<boolean> {
    try {
      const [result] = await pool.query(
        'UPDATE oauth_tokens SET is_active = FALSE, updated_at = NOW() WHERE provider = ? AND provider_user_id = ?',
        ['shopee', shopId]
      );
   ...
   */
  async listAuthorizedShops(userId?: number): Promise<ShopeeOAuthToken[]> {
    try {
