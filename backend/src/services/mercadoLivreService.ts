...
import { mockSearchResults, mockProductDetails, mockPriceHistory } from '../utils/mockMercadoLivre.js';
...
interface MercadoLivreSearchParams {
  ...
  userId?: number; // Para usar token OAuth do usuário
}

interface SellerItemsSearchParams {
  sellerId: number; // User ID do seller no ML
  
  // Paginação e tipo de busca
  limit?: number; // Default 50, max 100
  offset?: number; // Para paginação normal (não usar com search_type=scan)
  search_type?: 'normal' | 'scan'; // 'scan' para 1000+ itens
  scroll_id?: string; // Para paginação com scan
  include_filters?: boolean; // Retornar filters e available_filters
  ...
  sku?: string; // Buscar por seller_custom_field
  seller_sku?: string; // Buscar por SELLER_SKU
  missing_product_identifiers?: boolean; // true = sem product identifier
  ...
  // Filtros de Envio
  shipping_mode?: string; // ex: 'not_specified'
  ...
  labels?: Array<
    ...
    | 'accepts_mercadopago'
    | 'ending_soon'
    | 'with_mercadolibre_envios'
    | 'without_mercadolibre_envios'
    | 'with_low_quality_image'
    | 'with_free_shipping'
    | 'without_free_shipping'
    | 'with_automatic_relist'
    | 'waiting_for_payment'
    | 'suspended'
    | 'cancelled'
    | 'being_reviewed'
    ...
    | 'with_fbm_contingency'
    | 'with_shipping_self_service'
  >;
...
interface PublicSearchFilters {
  ...
  nickname?: string; // Buscar por nickname do seller
  ...
  // Envio
  shipping?: 'free'; // ou qualquer outro valor específico do site
...
class MercadoLivreService {
  ...
  private baseURL = 'https://api.mercadolibre.com'; // URL global da API (sem .br)
  private cache: Map<string, { data: any; timestamp: number }> = new Map();
  private cacheTTL = 3600000; // 1 hora em ms
  ...
  constructor() {
    ...
    this.apiClient.interceptors.response.use(
      (response) => {// Tratar 304 Not Modified
   ...
   */
  private async getUserOAuthToken(userId: number): Promise<string | null> {
    try {
      const [rows]: any = await db.query(
        'SELECT mercado_livre_token, mercado_livre_test_token, mercado_livre_test_user_id FROM users WHERE id = ?',
        [userId]
      );

      if (rows && rows.length > 0) {
        // Preferir token de teste se disponível (para contornar bloqueios de ngrok)
        if (rows[0].mercado_livre_test_token) {return rows[0].mercado_livre_test_token; // Retorna credenciais em base64
        }
        // Senão usar token principal
        if (rows[0].mercado_livre_token) {return rows[0].mercado_livre_token;
        }
      }
      return null;
    } catch (error: any) {      // Se a coluna não existe ainda, retornar null (a coluna será adicionada pela migração)
  ...
  async searchProducts(params: MercadoLivreSearchParams): Promise<Product[]> {
    ...
    try {
      ...
      if (token) {
        ...
        try {
          ...
          if (!token.includes(':')) {
            queryParams.access_token = token;} else {}
        }
      } else {}
      
      const response = await this.apiClient.get('/sites/MLB/search', {
        params: queryParams,
      });const products = response.data.results.map((item: any) => this.mapProduct(item));
      
      // Armazenar em cache
      this.cache.set(cacheKey, { data: products, timestamp: Date.now() });return products;
    } catch (error: any) {
      ...
      try {
        ...
        // Armazenar em cache
        this.cache.set(cacheKey, { data: mockProducts, timestamp: Date.now() });return mockProducts;
      } catch (mockError) {
   ...
   */
  async searchSellerItems(params: SellerItemsSearchParams, userToken?: string): Promise<any> {
    ...
    try {
      ...
      const headers: any = {};
      if (userToken) {
        headers.Authorization = `Bearer ${userToken}`;} else {}
      ...
      const response = await this.apiClient.get(`/users/${params.sellerId}/items/search`, {
        ...
        products: (response.data.results || []).map((itemId: string) => ({
          id: itemId,
   ...
   */
  async getSellerRestrictions(sellerId: number, userToken?: string): Promise<any> {
    try {
   ...
   */
  async getItemsMulti(itemIds: string[], attributes?: string[]): Promise<any> {
    try {
      if (itemIds.length === 0) {
        return [];
      }
      ...
      if (attributes && attributes.length > 0) {
        params.attributes = attributes.join(',');}const response = await this.apiClient.get('/items', {
        params,
      });
      ...
      this.cache.set(cacheKey, { data: processedResults, timestamp: Date.now() });return processedResults;
    } catch (error: any) {
   ...
   */
  async getUsersMulti(userIds: number[]): Promise<any> {
    try {
      if (userIds.length === 0) {
        return [];
      }
   ...
   */
  async searchPublicWithFilters(filters: PublicSearchFilters): Promise<any> {
    try {
      ...
      if (filters.price_min || filters.price_max) {
        let priceRange = '';
        if (filters.price_min) priceRange += filters.price_min;
        priceRange += '-';
        if (filters.price_max) priceRange += filters.price_max;
      ...
      const result = {
        ...
        results: (response.data.results || []).map((item: any) => this.mapProduct(item)),
        paging: response.data.paging,
        available_filters: response.data.available_filters || [],
   ...
   */
  async findCompetitors(productId: string): Promise<Product[]> {
    try {
   ...
   */
  async searchByCategory(categoryId: string, limit = 50): Promise<Product[]> {
    try {
      const response = await this.apiClient.get('/sites/MLB/search', {
        params: {
          category: categoryId,
      ...
      return response.data.results.map((item: any) => this.mapProduct(item));
    } catch (error) {
   ...
   */
  private mapProductDetails(item: any): ProductDetails {
    return {
      id: item.id,
      title: item.title,
      price: item.price,
   ...
   */
  private mapSellerInfo(seller: any): SellerInfo {
    return {
      id: seller.id,
  ...
  async testAPIConnection(): Promise<{ success: boolean; message: string; data?: any }> {
    try {const response = await this.apiClient.get('/sites/MLB/search', {
        params: {
          q: 'notebook',
          limit: 1,
        },
  ...
  async compareProducts(productIds: string[]): Promise<any> {
    try {
      const products = await Promise.all(
        productIds.map(id => this.getProductDetails(id).catch(() => null))
      );
