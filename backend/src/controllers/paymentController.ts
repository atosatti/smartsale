  ...
  try {
    ...
    try {
      ...
      const [users] = await connection.execute(
        'SELECT id, stripe_customer_id, email FROM users WHERE id = ?',
        [userId]
      );
...
export async function confirmPayment(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      const stripeChargeId = charge?.id || (paymentIntent as any).latest_charge || null;
      let stripeInvoiceId = (paymentIntent as any).invoice || null;
      
      // Extrair informações do cartão
      const stripePaymentMethodId = (paymentIntent as any).payment_method;
      let lastFour = null;
      let cardBrand = null;
      ...
      } else if ((paymentIntent as any).payment_method_details?.card?.last4) {
        lastFour = (paymentIntent as any).payment_method_details.card.last4;
        cardBrand = (paymentIntent as any).payment_method_details.card.brand;
        cardExpMonth = (paymentIntent as any).payment_method_details.card.exp_month;
        cardExpYear = (paymentIntent as any).payment_method_details.card.exp_year;
      } else if (stripePaymentMethodId) {
        // Se ainda não temos, tentar buscar do payment_method
        try {
          const paymentMethod = await stripe.paymentMethods.retrieve(stripePaymentMethodId);
          lastFour = (paymentMethod as any).card?.last4 || null;
          cardBrand = (paymentMethod as any).card?.brand || null;
          cardExpMonth = (paymentMethod as any).card?.exp_month || null;
          cardExpYear = (paymentMethod as any).card?.exp_year || null;
        } catch (e) {
          ...
          if ((existingMethod as any[]).length > 0) {
            ...
            const [result] = await connection.execute(
              `INSERT INTO payment_methods 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)`,
              [userId, stripePaymentMethodId, 'card', cardBrand, lastFour, cardExpMonth, cardExpYear]
            );
            paymentMethodDbId = (result as any).insertId;
          }
        } catch (e) {
      ...
      if ((paymentIntent as any).metadata?.invoiceType) {
        try {
          ...
          if (personName) {
            await connection.execute(
              `INSERT INTO invoices 
               ...
               VALUES (?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 7 DAY), ?, 'open')`,
              [userId, stripeInvoiceId, invoiceNumber, paymentIntent.amount / 100]
            );
          }
        } catch (e) {
    ...
    try {
      ...
      if (!subscription) {
        ...
        const [allSubs] = await connection.execute(
          'SELECT id, status, plan FROM subscriptions WHERE user_id = ? ORDER BY created_at DESC LIMIT 5',
          [userId]
        );
        console.log(`⚠️ Nenhuma assinatura ativa encontrada para user ${userId}. Assinaturas encontradas:`, allSubs);
        return res.status(404).json({ 
          error: 'Nenhuma assinatura ativa encontrada',
          debug: `Usuário tem ${(allSubs as any[]).length} assinatura(s)`
        });
      }
      ...
      const [feedbackResult] = await connection.execute(
        `INSERT INTO cancellation_feedback 
         ...
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
        [userId, subscription.id, reason || '', details || '', improvements || '', wouldReturn === true ? 1 : (wouldReturn === false ? 0 : null)]
      );

      const feedbackId = (feedbackResult as any).insertId;
      ...
      await connection.execute(
        `UPDATE users SET cancellation_feedback_id = ? WHERE id = ?`,
        [feedbackId, userId]
      );
 ...
 */
export async function getStripePaymentHistory(req: Request, res: Response) {
  try {
    const userId = (req as any).user?.id;
