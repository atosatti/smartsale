...
import { User, AdminAuditLog } from '../models/types.js';
...
export const getAllUsers = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [countResult] = await pool.query(
      ...
      } ${role ? 'AND role = ?' : ''} ${status === 'active' ? 'AND is_active = true' : ''} ${status === 'inactive' ? 'AND is_active = false' : ''}`,
      params
    );
...
export const getUserActivity = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Combinar e ordenar
    const activity = [...(searches as any[]), ...(payments as any[])]
...
export const getAuditLogs = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Total
    const [countResult] = await pool.query(
      `SELECT COUNT(*) as total FROM admin_audit_logs WHERE 1=1 ${
        adminId ? 'AND admin_user_id = ?' : ''
      } ${action ? 'AND action = ?' : ''}`,
      params
    );
  ...
  try {
    ...
    res.json({
      users: {
        ...
        twoFA: (twoFAUsers as any[])[0].count,
        verified: (verifiedUsers as any[])[0].count,
        byPlan: planBreakdown
      },
      revenue: {
        total: (revenue as any[])[0].total,
        lastMonth: (revenueMonth as any[])[0].total
      }
    });
  } catch (error) {
 ...
 */
export const getMercadoLivreIntegration = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user?.id) {
    ...
    const hasValidTestToken = !!user.mercado_livre_test_token;res.status(200).json({
      mainAccount: {
...
export const refreshMercadoLivreToken = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [users] = await pool.query(
      `SELECT mercado_livre_token, mercado_livre_refresh_token, mercado_livre_user_id FROM users WHERE id = ?`,
      [req.user.id]
    );
    ...
    if (!user.mercado_livre_refresh_token) {return res.status(400).json({ 
    ...
    await pool.query(
      `UPDATE users 
       SET mercado_livre_token = ?, 
           ...
           mercado_livre_token_expires_at = ?
       WHERE id = ?`,
      [data.access_token, data.refresh_token, expiresAt, req.user.id]
    );res.status(200).json({
      success: true,
    ...
    await pool.query(
      `UPDATE users 
       SET mercado_livre_token = ?, 
           ...
           mercado_livre_token_expires_at = ?
       WHERE id = ?`,
      [ml_token, ml_user_id, ml_refresh_token || null, expiresAt, req.user.id]
    );// Log na auditoria
 ...
 */
export const testExpireSubscription = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      ...
      const subscriptionId = (subscriptions as any[])[0].id;
      ...
      await logAdminAction(
        ...
        'subscription',
        { futureDate: expiryDate.toISOString() },
        req
      );
