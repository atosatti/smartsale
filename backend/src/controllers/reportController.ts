 ...
 */
export async function getCancellationReport(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      ((feedbacks as any[]) || []).forEach((fb: any) => {
        // Por razão
        stats.byReason[fb.reason] = (stats.byReason[fb.reason] || 0) + 1;

        // Voltaria a usar
        if (fb.would_return === 1) stats.wouldReturn.yes++;
        else if (fb.would_return === 0) stats.wouldReturn.no++;
        else stats.wouldReturn.unknown++;

        // Este mês
        if (new Date(fb.created_at) >= thisMonth) stats.thisMonth++;

        // Esta semana
        if (new Date(fb.created_at) >= thisWeek) stats.thisWeek++;
      });
 ...
 */
export async function getCancellationFeedback(req: Request, res: Response) {
  try {
    const userId = (req as any).user?.id;
    const userRole = (req as any).user?.role;
    ...
    try {
      ...
      const result = (feedback as any[])[0];
      if (!result) {
 ...
 */
export async function exportCancellationReport(req: Request, res: Response) {
  try {
    const userId = (req as any).user?.id;
    const userRole = (req as any).user?.role;
    ...
    try {
      ...
      const rows = ((feedbacks as any[]) || []).map((fb) => [
        fb.email || 'N/A',
