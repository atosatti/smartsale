    ...
    if (search_mode === 'seller') {
      ...
      const sellerFilters: any = {
        ...
        ...Object.keys(otherFilters).reduce((acc, key) => {
          if (otherFilters[key] !== undefined) acc[key] = otherFilters[key];
          return acc;
        }, {} as any),
      };
 ...
 */
export const findCompetitors = async (req: Request, res: Response) => {
  try {
    const { productId } = req.params;
...
export const searchByCategory = async (req: Request, res: Response) => {
  try {
    ...
    }const products = await mercadoLivreService.searchByCategory(
      categoryId,
 ...
 */
export const saveProduct = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO products 
       ...
       VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [userId, name, description || '', productId, price || 0, JSON.stringify(seller)]
    );
 ...
 */
export const getSavedProducts = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
